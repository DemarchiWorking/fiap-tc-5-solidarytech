...
| **F0.3b** | **SCA/SAST** com Trivy e Sonar | **Trivy em 2 camadas** (filesystem + imagem), `CRITICAL` bloqueia o push; **gosec** (Go) e **bandit** (Python) barram severidade alta; SonarCloud é opcional (roda só com `SONAR_TOKEN`) | `.github/workflows/ci-*.yml` | Print da execução **reprovada** pelo Trivy + [`devsecops-auditoria.txt`](07-evidencias/devsecops-auditoria.txt) | Demo Tech (item 1) | 🔴 | ✅ |
...
| **F3.z** | **Gestão de incidentes (PagerDuty)** — herdado da Fase 4 pela Regra de Ouro | Alerta `severity: page` abre incidente automaticamente, via `pagerduty_configs.routing_key_file` | `gitops/addons/kube-prometheus-stack/values.yaml` · `scripts/bootstrap-cluster.sh` | Print do incidente aberto **pelo alerta** | Demo Tech | 🔴 | ⚠️ |
...
| **E1.3** | Pipelines DevSecOps | Workflows versionados com testes + SAST (gosec/bandit) + Trivy | `.github/workflows/` | 🔴 | ✅ |
...
| Frente | Requisitos | ✅ | 🟨 falta print/execução | ⚠️ depende de credencial externa |
|---|---|---|---|---|
| F0 — Fundação | 9 | 9 | 0 | 0 |
| F1 — SRE | 6 | 6 | 0 | 0 |
| F2 — FinOps | 5 | 5 | 0 | 0 |
| F3 — ITSM/AIOps | 6 | 2 | 2 (Watchdog, self-heal disparado por alerta) | 2 (PagerDuty, Discord) |
| F4 — Segurança/DR | 4 | 4 | 0 | 0 |
| Entregáveis | 12 | 8 | 4 (vídeo e link do vídeo) | 0 |
| **Total** | **42** | **34** | **6** | **2** |
...
| 2026-09-24 | Auditoria final | Status real após o reprovisionamento na conta nova; F0.3b deixa de exigir SonarCloud (nunca rodou: SKIPPED sem token) e passa a citar o SAST que roda — gosec e bandit |
| 2026-09-24 | Validação contra o enunciado | F4.2a com **restore executado** (PVC do Prometheus recuperado de snapshot em 13 s) e backup de volumes comprovado; F4.x com revisão de segurança (IMDS bloqueado para pods que não usam AWS). Visão da banca em [`11-validacao-contra-o-enunciado.md`](11-validacao-contra-o-enunciado.md) |
